...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if(transition == "S"):
            ...
            tuple_la = (self.stack[-1], self.stack[-2])
            self.dependencies.append(tuple_la)
            self.stack.pop(-2)
        else:
            tuple_ra = (self.stack[-2], self.stack[-1])
            self.dependencies.append(tuple_ra)
            self.stack.pop(-1)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while(len(unfinished_parses)!= 0):
        ...
        # perform the predicted transition o each partial parse
        for parse, trans in zip(mini_batch, transitions):
            parse.parse_step(trans)
            # if the partial parse buffer in empty then remove that partial parse
